...
| [`spec/why.md`](spec/why.md) | V | The reason behind every Part II rule â€” each one the scar of a real bug. | **Before changing any Part II rule.** |
...
| [`spec/decisions.md`](spec/decisions.md) | â€” | **All 234 decisions. 228 ANSWERED, 2 PARKED, 2 DEFERRED, 2 HALF RULED, 0 BUILT NEVER RULED, 0 OPEN.** Counted, not typed â€” `scripts/decision-count.sh --check`. | Before proposing anything. |
| [`attic/lessons.md`](attic/lessons.md) | â€” | Thirty-one lessons, each the residue of a shipped defect. For a person, once. | **Never.** It says so at the top, and it means agents. |

**Parts VIIIâ€“XIII were proposal documents â€” artifacts and channels (`D1`â€“`D17`), `when`
variables (`W1`â€“`W14`), rebuild and caches (`K1`â€“`K18`), `firewall:` (`N1`â€“`N7`), secrets
(`T1`â€“`T7`) and the next round (`U1`â€“`U39`). Every one of those decisions is ruled and every
rule they produced is in Part II, so the documents were deleted (`Y21`). The IDs still resolve:
they are entries in the register.

**Where the work stands (updated 2026-07-26).** Phases 0â€“6 are built and the container matrix
(ubuntu/fedora/arch/alpine/tools) is green, run for real. **Phase 7 and the entire U-series
backlog are built** â€” the provider mechanism (snapshots U27/U28/U29, init U36, storage U30,
secrets U38), and the language-power features (module parameters U32, generated declarations U33,
user verbs U35, `repl` U34). **The last two open items â€” `D5` and U27's "built-ins become snapshot
rows" half â€” were cleared to build by the owner decision session 2026-07-26 and are built now, not
...
install and the Linux snapshot providers' live restore. **VI.0 â€” the bug that removed software
with no guard, no plan and no count, and that `--dry-run` performed â€” is FIXED** (S24/S25,
2026-07-23, verified 2026-07-24).

**On "the suite is green" â€” read this before quoting it.** Green here had always meant *on the
developer's Windows box*, and on 2026-07-26 that turned out to be load-bearing. `origin/main` was
112 commits behind `HEAD` until that day â€” the entire U-series, D5 and the provider mechanism had
...
(**S33** â€” it passes wherever a global git identity exists), Linux and macOS on a test asserting
Windows path semantics everywhere (**S34** â€” nobody had seen it, because the first red job hid the
...
suite is now green on Linux too, proven in a container with no git identity â€” 1,307 tests, 0
failures â€” including the `tests/` binaries that `cargo test --lib` never reaches.** That last
...
round trip is measured per host class rather than asserted here â€” the ratchet in
...
been run against a real package manager â€” 7 per distro image, 18 in the `tools` image" while the
...
`cfg!(target_os = â€¦)`. 48 on Windows and 56 on Ubuntu are both correct answers to the second
question. The 62 is asserted against the code by
`tests/backend_count_matches_the_spec_tests.rs` â€” a number in prose is a copy, and this one had
...
soft=6`), which is what `brew` â€” the one manager `release-check.sh`'s Darwin branch sweeps for
real â€” moves from *compiled* to *exercised*, and takes the count of backends that have ever run
against a real package manager from 22 to **23**. The destructive effectors â€” btrfs/zfs/lvm
restore, D5's `dpkg -i`/`rpm -U` handoff, U30 storage removal â€” are argv-tested and unrun.
...
> yet gone green"*, while the session record for 2026-07-27 â€” four lines further down the same
> tree â€” had the green run in it. The `62` beside it was right the whole time, because
> `tests/backend_count_matches_the_spec_tests.rs` asserts it against the code and this had
> nothing. **Where this file is checked it is true, and where it is prose it is not** â€” so read
...
[`spec/plan.md`](spec/plan.md). It is the only list of build state** â€” the register says whether
...
## PROMPT â€” read this first, then follow it

You are implementing Shall v7 on `main` â€” the sole branch â€” at `C:\Users\Administrator\Videos\Nexus\shall`.
...
1. **Part II is canonical.** If the code disagrees with Part II, the code is wrong. If Part
   II seems wrong, **stop and ask** â€” do not fix it yourself.
2. **Never change a Part II rule without reading its Part V entry first.** Each is the scar
   ...
   Part V says why. If Part V doesn't cover your case, that is a real gap â€” **ask.**
3. **Build without stopping for permission â€” and stop for exactly four things** (owner ruling,
   ...
   options that is invisible from outside the program â€” make the call and put the reasoning in the
...
4. **Never remove a feature without asking**, even one this document doesn't mention. Some
   may be genuinely important. The deletion list in II.17 is already approved â€” anything
   beyond it is a question.
5. **Do not invent a rule; do decide a detail.** If the spec doesn't say and the answer would be
   *visible from outside the program*, it is a gap â€” ask. If it is invisible from outside, decide
   ...
   **When a question is answered, the ruling ships in the same commit** â€” rewritten into
   ...
   **The same rule for a *finding*, and for the same reason.** A review round's output is a diff
   to `target-state.md` plus a test that fails without the fix â€” not a new dated document.
...
9. **A âœ… is earned by a command, not by a belief.** Rule 8 was already here, in these words,
   and **Phases 0 and 1 were both marked âœ… while untrue anyway** â€” so the rule is not
   ...
   done when the greps are quiet, not when the new thing works** â€” Phase 0 and Phase 1 both
...
10. **At every phase change, run Part VII's audit section.** It is a list of commands, not
    prose. Delete each finding as its command goes quiet â€” **in the same commit as the fix**,
...
11. **A green suite is not success. It is the absence of one kind of failure.** The tests
    ...
    rather than become the ninth. **Nothing in this document is verified by `cargo test`** â€”
    ...
    never "do the tests pass?" It is "did I do what the plan said, in full?"** â€” and that is
...
- **Phase 2 cannot be split, and the branch is red for a long stretch.** That is expected. Do
  **not** run the old and new models side by side behind a flag â€” that is the exact "two ways
...
- Every phase has an **exit condition**. Meet it before moving on. The exit condition is the
  bar â€” **not the test suite** (rule 11). Read the Exit lines and notice what they actually
  ...
  them. Phase 1's Exit is the one that reads like tests â€” "unit tests for every grammar rule
  above, including every error case" â€” and note that it names a *surface to cover*, not a
...
- **A comment states a constraint the code can't show. Nothing else.** Not what the line does.
  ...
  *(139 in the first draft; ~884 across 2,147 comment blocks on 2026-07-16. **Re-measured
  ...
  figure is historical, not current. The marketing/self-congratulation subset the R1â€“R23 and F5
  passes swept is now confirmed clean â€” a grep for the sales vocabulary (`blazing`, `world-class`,
  `enterprise-grade`, `mission-critical`, `seamless`, `bulletproof`, â€¦) finds nothing; the only two
  `magic` hits use the word pejoratively (V.83's "deciding by extension is magic that silently
  writes plaintext"), which is a constraint, not praise. What no grep can measure â€” a comment that
  narrates the line below it rather than stating a constraint â€” stays a per-comment judgement call,
...
- **Stale status drifts *both* ways.** This session the HEAD header lied *downward* â€” it said
  "Phases 3â€“6 not started" while a dozen Phase 3â€“5 items were done with commits behind them.
  Re-run the command; never trust a status line's direction. (Reinforces rules 9â€“11.)
- **`R1â€“R23` are owner-approved fixes â€” all done 2026-07-19. `SEC1â€“SEC7` were recorded
  vulnerabilities held back for a decision, and that decision has been made: all seven are now
  closed** (SEC1/SEC2 landed 2026-07-19, SEC4â€“SEC6 the same day, SEC7 deleted as dead code,
  SEC3's confinement half ruled **won't-fix** with only the outside-home confirmation built).
  *This bullet said "NOT yet decided â€” do not implement a SEC fix until the owner rules" for a
  ...
  a fix** â€” the shape of the defence is the owner's call, because every one of these has a
...
- **A "feature" that hand-rolls its own transaction/graph parallel to `sync` is a second engine to
  ...
  file, sync" â€” and teleport's private transaction *bypassed the guard* (a real safety hole). When
  ...
  **The follow-up matters as much as the finding:** what had to go was the private *engine*, not
  the verb. `teleport` was later re-added as `retarget` + `handle_sync` â€” a line edit that syncs,
  behind the guard like everything else â€” and it is in II.8's table today. `shim` did not come
...
review â€” including one whose entire purpose was hunting false claims in this file â€” read II.10's
...
1. **A list is an assertion about what is absent, and nothing verifies that half.** "Every X does
   Y â€” A, B, C" is checked by reading A, B and C, which is why the check always passes. When a
...
- **The security soft spot was the download/link backends, and that batch has landed.** The core
  was already safe â€” every PM command is argv (no `sh -c`), the II.12 hook ledger is enforced on
  ...
  **SEC1** `@bin` confinement (`[guard] confine_bin`, default on), **SEC2** HTTPS + checksum by
  default with `@allow_http` and `@unverified` as separate, never-implied opt-outs, **SEC4â€“SEC6**
  the injection/module-name hardening, **SEC7** the dead Lua exec path deleted. **SEC3 is decided
  as won't-fix:** `@target` stays unconfined â€” placing files outside `$HOME` is the feature â€” and
  ...
  **U31 is built** (2026-07-26 ruling, landed): a health-check command is argv from the config,
  ...
  change runs â€” the one runnable thing in the tree the ledger does not see would have been
